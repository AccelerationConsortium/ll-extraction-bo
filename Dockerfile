...
# and `pip install ll-extraction` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "ll-extraction", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n ll-extraction -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "ll-extraction", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "ll-extraction", "YOUR_CONSOLE_SCRIPT"]
